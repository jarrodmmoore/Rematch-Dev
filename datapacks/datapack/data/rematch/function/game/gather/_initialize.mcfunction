scoreboard players set @e[name=Global] gameState 1
...
scoreboard players set @e[name=Global] extraCDelay 100
...
scoreboard players reset @e[name=Global] eventScore
execute if entity @e[tag=settings_stand,scores={eventsToggle=1}] run scoreboard players operation @e[name=Global] eventScore = @e[tag=arenaStand,scores={arenaID=1..7},sort=random,limit=1] arenaID

#reset player ID's
function rematch:give_all_player_id

#summon player currency armor stands
scoreboard players set @e[name=Global] itemID 2
...
scoreboard players reset @e[name=Global] itemID
...
scoreboard players set @e[name=Global] countdown 1
return 1