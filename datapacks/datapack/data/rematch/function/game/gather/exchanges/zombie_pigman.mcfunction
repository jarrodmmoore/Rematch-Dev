...
scoreboard players add @e[tag=currencystand,scores={math=0}] math2 20
execute if entity @e[scores={eventScore=1},name=Global] run scoreboard players operation @e[tag=currencystand,scores={math=0}] math2 *= @e[limit=1,name=Global] x2
...
scoreboard players reset @s kill_zomb_pigman
