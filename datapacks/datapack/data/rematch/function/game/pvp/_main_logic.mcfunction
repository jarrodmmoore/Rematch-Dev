...
scoreboard players set @e[scores={oTimer=21..},name=Global] oTimer 1

#item logic
function rematch:game/pvp/items/_items_main

#timer logic
scoreboard players remove @e[name=Global] timeRemaining 1
execute if entity @e[scores={timeRemaining=1..1220},name=Global] run function rematch:game/pvp/time_bossbar
execute if entity @e[scores={timeRemaining=..0},name=Global] run bossbar set minecraft:timeremaining players
execute if entity @e[scores={timeRemaining=..0},name=Global] run function rematch:game/pvp/deathmatch/deathmatch_main
...
execute if entity @e[tag=!darkHUD,name=Global] as @a[gamemode=adventure,team=!spectators] unless entity @s[nbt={SelectedItem:{components:{"minecraft:custom_data":{Tags:["droptouse"]}}}}] unless entity @s[nbt={SelectedItem:{components:{"minecraft:custom_data":{Tags:["deployonground"]}}}}] unless entity @s[nbt={SelectedItem:{components:{"minecraft:custom_data":{Tags:["holdinventory"]}}}}] run function rematch:text/pvp/self_lives_display
...
#end game logic
execute if entity @e[tag=!teamplay,name=Global] run function rematch:game/pvp/end_game_logic_ffa
execute if entity @e[tag=teamplay,name=Global] run function rematch:game/pvp/end_game_logic_teamplay
return 1