...
tag @a[tag=requestRedTeam,tag=!teamassigned,sort=random,limit=1,team=!spectators] add ichooseyou
...
execute unless entity @a[tag=ichooseyou] run tag @a[tag=requestBlueTeam,tag=!teamassigned,sort=random,limit=1,team=!spectators] add ichooseyou


#now give the chosen player all the proper stuff..
tag @a[tag=ichooseyou] remove requestRedTeam
tag @a[tag=ichooseyou] add teamassigned
team join redplayers @a[tag=ichooseyou]
function rematch:text/pvp/team_assign_red
