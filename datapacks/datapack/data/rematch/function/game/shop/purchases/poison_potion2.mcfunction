execute if entity @s[nbt={SelectedItem:{id:"minecraft:splash_potion",components:{"minecraft:potion_contents":{potion:"minecraft:strong_poison"}}}}] run tag @s add refund
execute unless entity @s[tag=refund] unless entity @s[scores={currency=100..}] run tag @s add reject
execute if entity @s[tag=refund] run function rematch:game/shop/refunds/poison_potion2
...
execute if entity @s[tag=!reject,tag=!refund] run give @s minecraft:splash_potion[minecraft:custom_name='{"bold":true,"color":"green","text":"Poison II Potion"}',minecraft:item_name='{"extra":["Poison II Potion"],"text":"EMU"}',minecraft:potion_contents={potion:"minecraft:strong_poison"},minecraft:custom_data={emu_lock_name:'{"bold":true,"color":"green","text":"Poison II Potion"}'}] 1
execute if entity @s[tag=!reject,tag=!refund] run tellraw @s ["",{"text":"\u2714 Purchased","color":"dark_green","type":"text"},{"text":" [Poison II Potion]","bold":true,"color":"green","type":"text"}]
