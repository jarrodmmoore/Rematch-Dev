execute if entity @s[nbt={SelectedItem:{id:"minecraft:splash_potion",components:{"minecraft:potion_contents":{potion:"minecraft:strong_harming"}}}}] run tag @s add refund
execute unless entity @s[tag=refund] unless entity @s[scores={currency=80..}] run tag @s add reject
execute if entity @s[tag=refund] run function rematch:game/shop/refunds/damage_potion2
...
execute if entity @s[tag=!reject,tag=!refund] run give @s minecraft:splash_potion[minecraft:custom_name='{"bold":true,"color":"dark_purple","text":"Damage II Potion"}',minecraft:item_name='{"extra":["Damage II Potion"],"text":"EMU"}',minecraft:potion_contents={potion:"minecraft:strong_harming"},minecraft:custom_data={emu_lock_name:'{"bold":true,"color":"dark_purple","text":"Damage II Potion"}'}] 1
execute if entity @s[tag=!reject,tag=!refund] run tellraw @s ["",{"text":"\u2714 Purchased","color":"dark_green","type":"text"},{"text":" [Damage II Potion]","bold":true,"color":"dark_purple","type":"text"}]
